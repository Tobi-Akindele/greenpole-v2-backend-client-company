...
public interface UserPermissionRepository extends JpaRepository<UserPermission, Long>, PagingAndSortingRepository<UserPermission, Long> {
    ...
    @Query(value = "SELECT up.id as id, permission_id, user_id, p.value as val FROM user_permissions up INNER JOIN permissions p ON p.id = up.permission_id WHERE 'value' = ?1 AND user_id = ?2", nativeQuery = true)
