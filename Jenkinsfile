        ...
        stage('This is Docker Build') {
            ...
            steps {
                script {
                    sshagent(['jenkins']) {
                        sh '''
                            if sshpass -p admin002  sudo docker build -t greenpole.azurecr.io/client-company:latest -f Dockerfile . ; then
        ...
        stage ('Integration Testing'){
            ...
            steps {
                script {
                    sshagent(['jenkins']) {
                        containerID = sh (
                                        script: "sshpass -p admin002  sudo docker run -d --name client-company -e SPRING_DATASOURCE_URL=jdbc:postgresql://104.40.226.227:5432/greenpole -e SPRING_DATASOURCE_PASSWORD=abi_step1ola -e SPRING_DATASOURCE_USERNAME=abiola002 -p 8289:8086 greenpole.azurecr.io/client-company:latest",
                                    returnStdout: true
                                    ).trim()
            ...
            steps {
                script {
                    sshagent(['jenkins']) {
                                        ...
                                        push_output = sh (
                                                        script: "sshpass -p admin002  sudo docker push  greenpole.azurecr.io/client-company:latest",
                                                    returnStdout: true
                                                    ).trim()
